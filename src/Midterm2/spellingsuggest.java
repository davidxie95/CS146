...
class spellingsuggest {
	...
	public spellingsuggest(String file) throws IOException {
		try {
			...
			for (String temp = ""; temp != null; temp = in.readLine()) {
				...
				while (m.find()) {
					DBWords.put((temp = m.group()),
							DBWords.containsKey(temp) ? DBWords.get(temp) + 1
									: 1); // This will serve as an indicator to
	...
	public final String correct(String word) {
		...
		return candidates.size() > 0 ? candidates.get(Collections
				.max(candidates.keySet()))
				: "Sorry but no possible corrections found!";
	}

	public static void main(String[] args) throws IOException {
		if (args.length > 0) {
			System.out.println((new spellingsuggest(
					"wordprobabilityDatabase.txt")).correct(args[0]));
		}
	}
}